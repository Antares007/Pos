...
namespace POS.ViewModels.Sale.Printing
{
    ...
    {
        public async void PrintReceipt(CQQ cq)
        {
      
            var biSource = new BitmapImage();

            var ms = await (await new HttpClient().GetAsync(cq.GetAttr(".logo", "src"))).Content.ReadAsStreamAsync();
            ...
            var items = cq.All(".yvela .chekisKhazi", cqq => new PartReceiptItem()
                {
            ...
            var amountItems = cq.All(".yvela .jamisKhazi", cqq => new PartReceiptAmountItem()
                {
                    Name = { Text = cqq.GetText(".dasakheleba") },
